...
const CommunityArticleDetailPage = () => {
  ...
  const fetchArticles = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/whole/${articleid}`, {
        headers: {
          Authorization: `Bearer ${user.accessToken}`,
        },
      });
      const data = response.data.data;
      setArticles({ ...data, comments: data.comments || [] });
    } catch (error) {
      console.error(error);
          ...
          {isEditOpen ? (
            ...
            <form onSubmit={handleEditSubmit}>
              <input
                type="text"
                value={board.title}
                onChange={(e) => setArticles({ ...board, title: e.target.value })}
              />
              <textarea
                value={board.content}
                onChange={(e) => setArticles({ ...board, content: e.target.value })}
              />
